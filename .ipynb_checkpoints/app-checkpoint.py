...
def handle_text_message(event):
    try:
        ...
        # Save the user's message to the conversation memory
        memory.save_context(
        ...
        # Save the model's response to the conversation memory
        memory.save_context(
            {"input": ""},
            {"output": f"assistant: {reply_message}"}
        )
...
def handle_image_message(event):
    try:
        ...
        # Call OpenAI API's GPT-4 model to analyze the image
        response = openai.chat.completions.create(
            model="gpt-4o",
            messages=[
